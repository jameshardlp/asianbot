...
def is_sentence_complete(sentence: str) -> bool:
    """
    Проверяет, является ли предложение логически завершённым.
    Проверяет наличие глагола, подлежащего и завершённой мысли.
    """
    if not sentence:
        return False
    
    # Убираем знаки препинания в конце для проверки
    clean = re.sub(r'[.!?]$', '', sentence).strip()
    
    # Если предложение слишком короткое — скорее всего не завершено
    if len(clean.split()) < 3:
        return False
    
    # Проверяем наличие глагола (примитивная проверка)
    verbs = ['быть', 'стать', 'являться', 'иметь', 'делать', 'сказать', 'пойти',
    ...
    has_verb = any(verb in clean.lower() for verb in verbs)
    
    # Проверяем наличие подлежащего
    has_subject = bool(re.search(r'\b(я|ты|он|она|оно|мы|вы|они|это|тот|всё|все|кто|что)\b', clean, re.IGNORECASE))
    
    # Если предложение длинное и имеет смысл — считаем завершённым
    if len(clean) > 30:
        return True
...
def validate_caption(text: str, min_length: int = 700, max_length: int = 1023) -> tuple:
    ...
    if last_sentence:
        ...
        # Проверяем, что последнее предложение содержит больше 1 слова
        ...
        # ===== НОВОЕ: проверка логической завершённости =====
...
def get_fallback_caption() -> str:
    fallbacks = [
        ...
        "Пиздец, я только что понял, что жизнь удалась. Я в Instagram, рядом молодая азиатка в обычном образе.",
