...
def is_sentence_complete(sentence: str) -> bool:
    ...
    Исправленная версия — проверяет смысловую завершённость.
    """
    if not sentence:
        return False
    ...
    # Если предложение слишком короткое — не завершено
    if len(clean.split()) < 3:
        return False
    
    # Проверяем, заканчивается ли предложение на вводные конструкции
    incomplete_endings = [
    ...
    # Проверяем наличие глагола (дополненный список)
    verbs = [
    ...
    has_subject = bool(re.search(r'\b(я|ты|он|она|оно|мы|вы|они|это|тот|всё|все|кто|что|который|которые|которое)\b', clean, re.IGNORECASE))
    
    # Проверяем, что предложение не обрывается на предлоге или союзе
    last_word = clean.split()[-1] if clean.split() else ''
    incomplete_words = ['и', 'а', 'но', 'да', 'или', 'либо', 'за', 'перед', 'под', 'над', 'без', 'для', 'про', 'через', 'между', 'среди']
...
def validate_caption(text: str, min_length: int = 700, max_length: int = 1023) -> tuple:
    ...
    if last_sentence:
        ...
        # Проверяем логическую завершённость
